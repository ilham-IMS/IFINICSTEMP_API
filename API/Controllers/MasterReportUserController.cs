...
namespace API.Controllers
{
  ...
  public class MasterReportUserController : BaseController
  {
    ...
    [HttpPost("Insert")]
    public async Task<ActionResult> Insert(List<MasterReportUser> modules)
    {
      try
      {
        int results = await _service.Insert(modules);
        return ResponseSuccess(new { }); // Mengembalikan hasil dari penyisipan
