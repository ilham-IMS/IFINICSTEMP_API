...
namespace DAL
{
  public class DynamicReportTableExtRepository : BaseRepository, IDynamicReportTableExtRepository
  {
    private string tableName = "dynamic_report_table_ext";
    ...
    public async Task<List<ExtendModel>> GetRowForParent(IDbTransaction transaction, string parentID)
    {
      ...
      string query = $@"
        select
          id                        as ID
          ,dynamic_report_table_id         as ParentID
          ,keyy                     as Keyy
          ,value                    as Value
        from
        {tableName}
        where
          dynamic_report_table_id = {p}ID";
    ...
    public async Task<int> UpdateByID(IDbTransaction transaction, ExtendModel model)
    {
      ...
      string query = $@"
        ...
        where
          dynamic_report_table_id  = {p}ParentID
        and
          keyy = {p}Keyy";
