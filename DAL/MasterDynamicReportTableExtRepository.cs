...
namespace DAL
{
  public class MasterDynamicReportTableExtRepository : BaseRepository, IMasterDynamicReportTableExtRepository
  {
    private string tableName = "master_dynamic_report_table_ext";
    ...
    public async Task<List<ExtendModel>> GetRowForParent(IDbTransaction transaction, string parentID)
    {
      ...
      string query = $@"
        select
          id                        as ID
          ,master_dynamic_report_table_id         as ParentID
          ,keyy                     as Keyy
          ,value                    as Value
        from
        {tableName}
        where
          master_dynamic_report_table_id = {p}ID";
    ...
    public async Task<int> UpdateByID(IDbTransaction transaction, ExtendModel model)
    {
      ...
      string query = $@"
        ...
        where
          master_dynamic_report_table_id  = {p}ParentID
        and
          keyy = {p}Keyy";
