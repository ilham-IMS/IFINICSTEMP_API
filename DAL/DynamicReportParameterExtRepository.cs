...
namespace DAL
{
  public class DynamicReportParameterExtRepository : BaseRepository, IDynamicReportParameterExtRepository
  {
    private string tableName = "dynamic_report_parameter_ext";
    ...
    public async Task<List<ExtendModel>> GetRowForParent(IDbTransaction transaction, string parentID)
    {
      ...
      string query = $@"
        select
          id                        as ID
          ,dynamic_report_parameter_id  as ParentID
          ,keyy                     as Keyy
          ,value                    as Value
        from
        {tableName}
        where
          dynamic_report_parameter_id = {p}ID";
        ...
        public async Task<int> UpdateByID(IDbTransaction transaction, ExtendModel model)
        {
            ...
            string query =
                $@"
                            ...
                            where
                                dynamic_report_parameter_id  = {p}ParentID
                            and
                                keyy = {p}Keyy";
